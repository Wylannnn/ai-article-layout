...
import {
  ...
  CARD_PLATFORMS, CARD_THEMES_BY_CATEGORY, CARD_THEMES_EXTRAS, CardTheme, CardDesignToken,
} from "@/types";
import { getCardTheme, getThemeCSS, themeToToken } from "@/lib/card-themes";
...
import { generateDesign } from "@/lib/card-design";
...
export default function CardDeckPanel({ analysis, articleText, showToast, onBack }: Props) {
  ...
  const [designStyle, setDesignStyle] = useState<"ai" | "preset">(saved?.designStyle ?? "ai");
  ...
  const [aiDesignStep, setAiDesignStep] = useState<"idle" | "designing" | "done">("idle");
  ...
  const save = useCallback(() => {
    const s: CardSettings = {
      ...
      themeId, designStyle, contentStyle, accountHandle: handle, endingSlogan: slogan,
    };
    saveCardSettings(s);
  }, [platformId, customW, customH, themeId, designStyle, contentStyle, handle, slogan]);
  ...
  const generate = useCallback(async () => {
    ...
    setProgress(0);
    setGenerated([]);
    setAiDesignStep("designing");

    try {
      // 1. Generate (or resolve) design token
      let token: CardDesignToken;
      if (designStyle === "ai") {
        token = await generateDesign({
          ...
          sections: analysis.sections.map((s) => ({
            title: s.title,
            estimatedType: estimateSectionType(s.content),
          })),
          targetPlatform: platformId,
        });
      } else {
        token = themeToToken(activeTheme);
      }
      setAiDesignStep("done");

      // 2. Build and render cards
      const cardPages = buildCardDeck(
        analysis, articleText, platform, token,
  ...
  }, [analysis, articleText, platform, activeTheme, designStyle, contentStyle, handle, slogan, platformId, save, showToast]);
    ...
    <div className="h-full overflow-y-auto" style={{ background: "var(--bg, #ffffff)" }}>
      <div className="p-5 space-y-4">
          ...
          <div>
            <Label>设计方式</Label>
            <div className="flex gap-2">
              <button onClick={() => { setDesignStyle("ai"); save(); }}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${designStyle === "ai" ? "font-semibold" : ""}`}
                style={{
                  borderColor: designStyle === "ai" ? "var(--text, #111)" : "var(--border, #e5e7eb)",
                  background: designStyle === "ai" ? "var(--bg-secondary, #f3f4f6)" : "transparent",
                  color: "var(--text, #111)",
                }}>
                🎨 AI 自动设计
              </button>
              <button onClick={() => { setDesignStyle("preset"); save(); }}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${designStyle === "preset" ? "font-semibold" : ""}`}
                style={{
                  borderColor: designStyle === "preset" ? "var(--text, #111)" : "var(--border, #e5e7eb)",
                  background: designStyle === "preset" ? "var(--bg-secondary, #f3f4f6)" : "transparent",
                  color: "var(--text, #111)",
                }}>
                🎨 预设主题
              </button>
            </div>
            {designStyle === "ai" && aiDesignStep !== "idle" && (
              <div className="flex items-center gap-2 mt-1.5 text-xs"
                style={{ color: aiDesignStep === "designing" ? "var(--text-secondary)" : "#22c55e" }}>
                {aiDesignStep === "designing" ? "⏳ AI 正在设计视觉风格..." : "✅ AI 设计完成"}
              </div>
            )}
            {designStyle === "ai" && (
              <p className="text-xs mt-1.5" style={{ color: "var(--text-tertiary, #9ca3af)" }}>
                AI 根据文章内容自动生成配色、字体、布局。每篇文章独一无二。
              </p>
            )}
          </div>

          {/* Theme picker (only for preset mode) */}
          {designStyle === "preset" && (
            <>
              <Label>配色方案</Label>
...
/** Estimate section type from content text */
function estimateSectionType(content: string): "article" | "steps" | "data" {
  const hasSteps = /(\d+[.、．]\s*|Step\s+\d+)/i.test(content.slice(0, 200));
  if (hasSteps) return "steps";
  const hasData = /(\d+[%x×]|\d+\.\d+|\d{3,})/.test(content);
