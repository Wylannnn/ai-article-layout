...
import { ArticleAnalysis, ArticleCategory, LayoutStyle, ProviderConfig, PROVIDER_META, TemplateId } from "@/types";
...
import { splitContentIntoBlocks, checkSplitIntegrity, pickStyleHintSnippet } from "@/lib/content-blocks";
import { injectContentBlocks, buildFallbackHTML } from "@/lib/inject-content";
import { runTemplatePipeline } from "@/lib/template-orchestrator";
...
export default function Home() {
  ...
  const [layoutMode, setLayoutMode]     = useState<"ai" | "template">("ai");
  ...
  const startLayout = async () => {
    ...
    console.time("【总耗时】完整排版流程");

    try {
      // ── Step 1-3: Analyze ──────────────────────────────────
      setStep(0, "active");
      console.time("【耗时】Step1-3 分析阶段(LLM调用)");
      const raw = await callAI(
        providerConfig,
        [{ role: "user", content: text.slice(0, 4000) }],
        { system: ANALYSIS_SYSTEM, maxTokens: 3000, jsonMode: true, prefill: "{" }
      );
      console.timeEnd("【耗时】Step1-3 分析阶段(LLM调用)");
      ...
      // ★ 从原文提取风格参考片段（最多300字），传给模型弥补看不见原文的同质化问题
      const styleHintSnippet = pickStyleHintSnippet(contentBlocks, 300);

      // ── Step 4: Generate HTML ────────────────────────────
      setStep(3, "active");

      if (layoutMode === "template") {
        // Template pipeline: AI semantic analysis → template render
        console.time("【耗时】Step4 模板排版流程");
        try {
          ...
          console.timeEnd("【耗时】Step4 模板排版流程");
          setHTML(result.html);
        } catch (templateErr) {
          console.error("Template pipeline error:", templateErr);
          // Fallback to basic template
          const fallbackHTML = buildFallbackHTML(a.title, contentBlocks);
          setHTML(fallbackHTML);
          showToast("⚠ 模板排版异常，已使用基础模板");
        }
        setProgress(100);
        setStep(3, "done");
      } else {
...
【风格参考】以下是文章各章节的原文片段（仅用于感受写作语气和节奏，正文内容将从占位符自动注入）：
${styleHintSnippet || "(无，请根据章节标题自行判断)"}`;

      let received = 0;
      console.time("【耗时】Step4 排版生成阶段(LLM流式调用)");
      await callAI(
        providerConfig,
      ...
      console.timeEnd("【耗时】Step4 排版生成阶段(LLM流式调用)");
      console.log("【数据】本次排版骨架实际生成字符数:", htmlAccRef.current.length);
      console.time("【耗时】cleanHTMLOutput(本地计算)");
      const skeleton = cleanHTMLOutput(htmlAccRef.current);
      console.timeEnd("【耗时】cleanHTMLOutput(本地计算)");
      // 先校验骨架完整性（截断检测）
      ...
      // 注入真实原文内容（替代模型自己写正文）
      console.time("【耗时】injectContentBlocks(本地计算，内容注入)");
      const { html: injectedHTML, report: injectionReport } = injectContentBlocks(skeleton, contentBlocks);
      console.timeEnd("【耗时】injectContentBlocks(本地计算，内容注入)");
          ...
          <div className="flex flex-col p-3 gap-3 md:min-h-full">
            ...
            <div>
              ...
              <div className="flex gap-1 p-0.5 rounded-lg" style={{ background: "var(--bg-tertiary)" }}>
                <button onClick={() => { setLayoutMode("ai"); if (analysis) setStyleDirty(true); }}
                  className={clsx(
                    "flex-1 py-1.5 rounded-md text-xs font-medium transition-all",
                    layoutMode === "ai" ? "bg-[var(--bg)] shadow-sm" : "text-[var(--text-tertiary)]"
                  )}
                  style={{ color: layoutMode === "ai" ? "var(--text)" : "var(--text-tertiary)" }}>
                  AI 排版
                </button>
                <button onClick={() => { setLayoutMode("template"); if (analysis) setStyleDirty(true); }}
                  className={clsx(
                    "flex-1 py-1.5 rounded-md text-xs font-medium transition-all",
                    layoutMode === "template" ? "bg-[var(--bg)] shadow-sm" : "text-[var(--text-tertiary)]"
                  )}
                  style={{ color: layoutMode === "template" ? "var(--text)" : "var(--text-tertiary)" }}>
                  模板排版
                </button>
              </div>
            </div>
            ...
            {layoutMode === "template" && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    { id: "editorial" as TemplateId, label: "杂志编辑风", desc: "深度长文 · 首字下沉 · 侧边目录" },
                    { id: "tutorial-minimal" as TemplateId, label: "教程文档风", desc: "步骤清晰 · 暗色代码块 · 极简排版" },
                  ]).map((t) => (
                    <button key={t.id} onClick={() => setTemplateId(t.id)}
                      className={clsx(
                        "flex flex-col items-start gap-0.5 p-2.5 rounded-lg text-xs transition-all border text-left",
                        templateId === t.id
                          ? "border-[var(--text)] bg-[var(--bg-secondary)]"
                          : "border-[var(--border)] hover:border-[var(--text-secondary)]"
                      )}>
                      <span className="font-medium text-xs" style={{ color: "var(--text)" }}>{t.label}</span>
                      <span style={{ color: "var(--text-tertiary)", lineHeight: 1.3 }}>{t.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
