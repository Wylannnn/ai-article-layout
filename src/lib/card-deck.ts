...
import { ArticleAnalysis, CardPlatformPreset, CardPage, ContentCardStyle, CardDesignToken } from "@/types";
...
function propsFromToken(width: number, height: number, token: CardDesignToken): CardTemplateProps {
...
export function buildCardDeck(
  ...
  const p = propsFromToken(width, height, token);
