...
- **start.bat**: 全面重写启动脚本，修复以下问题：
  - Python 3.13+ 默认注册 `py` 而非 `python`，原脚本直接调用 `python` 导致找不到
  - 启动失败时 cmd 窗口一闪而过，用户看不到错误信息
  - 端口 3000 被占用时直接崩溃，改为自动尝试 3000~3005
  - 新增服务启动后自动打开浏览器
- **start.command**: 端口扫描增加上限（3000~3005），避免无限循环；自动打开浏览器改为非阻塞

## v0.4.0

### ✨ Features

- 新增卡片图套装功能：完成智能排版后，可将文章一键转为适合小红书/抖音/朋友圈的卡片图片
  - 5 种平台尺寸预设：小红书 3:4、小红书 1:1、抖音 3:4、抖音 9:16、朋友圈 1:1，支持自定义尺寸
  - 3 种内容排版模板：文章风（引用/段落）、步骤风（编号列表）、数据信息图风（统计卡片）
  - 配色方案自动匹配文章分类（科技/财经/旅行等），另附 2 套额外配色可选
  - 封面卡 + 正文卡 N 张 + 结尾引导卡完整套装
  - 支持单张下载和 ZIP 打包下载全部
  - 设置（平台/配色/账号名）自动保存到浏览器本地存储
- 新增 `card-paginate` 分页逻辑：按段落/步骤自动拆分文章到多张卡片，超长段落按字符数兜底拆分
- 新增 11 个单元测试覆盖分页与配色的纯函数部分

### 🛠 Improvements

- 卡片设计采用杂志风格模板系统：封面有色带+标题+摘要、正文有章节标记/引用/步骤/数据多种布局
- 零 AI 成本：卡片图排版完全基于本地 CSS 模板，不产生额外 API 费用
- 页面中顶部工具栏新增「卡片图套装」按钮（分析完成后可用），预览弹层展示每张卡片缩略图
